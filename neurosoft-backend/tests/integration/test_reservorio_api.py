...
def admin_token(client):
    r = client.post("/api/v1/auth/login",
                    json={"username": "admin", "password": "neurosoft2025"})
...
class TestReservorioList:
    ...
    def test_list_filtrado_infantil(self, client, auth_headers):
        r = client.get("/api/v1/reservorio/cuadros?poblacion=infantil",
    ...
    def test_list_filtrado_adulto_mayor(self, client, auth_headers):
        r = client.get("/api/v1/reservorio/cuadros?poblacion=adulto_mayor",
                       headers=auth_headers)
        assert r.status_code == 200
        j = r.json()
        assert j["total"] >= 8
    ...
    def test_list_filtro_inexistente(self, client, auth_headers):
        r = client.get("/api/v1/reservorio/cuadros?poblacion=extraterrestre",
                       headers=auth_headers)
        assert r.status_code == 200
...
class TestReservorioDetalle:
    def test_cuadro_existe(self, client, auth_headers):
        r = client.get("/api/v1/reservorio/cuadros/infantil/tdah",
                       headers=auth_headers)
        assert r.status_code == 200
        j = r.json()
        assert j["id"] == "tdah"
    ...
    def test_cuadro_no_existe_404(self, client, auth_headers):
        r = client.get("/api/v1/reservorio/cuadros/infantil/noexiste",
                       headers=auth_headers)
        assert r.status_code == 404
        assert "Disponibles" in r.json()["detail"]

    def test_grupo_no_existe_404(self, client, auth_headers):
        r = client.get("/api/v1/reservorio/cuadros/marte/tdah",
                       headers=auth_headers)
        assert r.status_code == 404
...
class TestReservorioSugerir:
    ...
    def test_sugerir_am_con_grober_severo(self, client, auth_headers):
        ...
        resultados = [
            {"test_id": "ViGroberML", "tipo_metrica": "memoria",
             "dominio_cognitivo": "Memoria", "z_equivalente": -2.8},
            {"test_id": "ViGroberRLT", "tipo_metrica": "memoria",
             "dominio_cognitivo": "Memoria", "z_equivalente": -2.5},
        ]
        r = client.get("/api/v1/reservorio/sugerir", params={
    ...
    def test_sugerir_infantil_con_tdah_screen(self, client, auth_headers):
        ...
        resultados = [
            {"test_id": "NiSNAP", "tipo_metrica": "tdah_screen",
             "dominio_cognitivo": "Atención", "z_equivalente": 2.0,
             "pd": 28},
            {"test_id": "NiWiscDC", "tipo_metrica": "atencion",
             "dominio_cognitivo": "Atención", "z_equivalente": -1.8},
            {"test_id": "NiWiscCl", "tipo_metrica": "ffee",
             "dominio_cognitivo": "Funciones Ejecutivas", "z_equivalente": -1.5},
        ]
        r = client.get("/api/v1/reservorio/sugerir", params={
            "resultados": json.dumps(resultados),
            "poblacion": "infantil",
