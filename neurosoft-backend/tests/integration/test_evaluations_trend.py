...
class TestEvaluationsTrend:
    ...
    def test_trend_meses_param(self, app_client, admin_token):
        r = app_client.get("/api/v1/evaluations/trend?meses=12", headers=_auth(admin_token))
        assert r.status_code == 200
        assert len(r.json()) == 12

    def test_trend_meses_fuera_de_rango_422(self, app_client, admin_token):
        r = app_client.get("/api/v1/evaluations/trend?meses=0", headers=_auth(admin_token))
        assert r.status_code == 422
        r2 = app_client.get("/api/v1/evaluations/trend?meses=99", headers=_auth(admin_token))
        assert r2.status_code == 422

    def test_trend_orden_cronologico(self, app_client, admin_token):
        r = app_client.get("/api/v1/evaluations/trend?meses=4", headers=_auth(admin_token))
        body = r.json()
