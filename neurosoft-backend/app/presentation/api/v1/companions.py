...
_RELACIONES_VALIDAS = ("madre", "padre", "hermano", "conyuge",
                       "hijo", "cuidador", "tutor", "otro")
...
@companions_router.get("", response_model=list[CompanionResponseDTO],
                       summary="Listar acompañantes de un paciente")
def list_companions(patient_id: str = Query(...), db: DbSession = None):
    ...
    rows = (db.query(CompanionORM)
            .filter_by(patient_id=patient_id)
            .order_by(CompanionORM.es_principal.desc(),
                      CompanionORM.created_at.desc())
            .all())
    return [_row_to_dto(r) for r in rows]


@companions_router.post("", response_model=CompanionResponseDTO, status_code=201,
...
def create_companion(patient_id: str, dto: CompanionDTO, db: DbSession):
    ...
    if dto.relacion and dto.relacion not in _RELACIONES_VALIDAS:
        raise HTTPException(400,
            detail=f"Relación inválida. Valores aceptados: {', '.join(_RELACIONES_VALIDAS)}")
...
@companions_router.put("/{companion_id}", response_model=CompanionResponseDTO,
...
def _row_to_dto(row):
    return CompanionResponseDTO(
        ...
        relacion=row.relacion, documento=row.documento,
