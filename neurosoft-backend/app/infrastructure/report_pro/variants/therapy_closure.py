...
class TherapyClosureGenerator(NeuroPDFGeneratorPro):
    ...
    def _build_pages(self, c, data) -> None:
        ...
        y = self._ensure_room(c, y, 150, data)
        ...
        if getattr(data, "therapy_risk_history", None):
            y = self._ensure_room(c, y, 100, data)
    ...
    def _draw_marco_terapeutico(self, c, data, y: float) -> float:
        ...
        for label, value in rows:
            draw_text(c, label, L.margin_left, y - 4,
                      font_name=FONT_SANS_BOLD, size=8, color=SLATE)
            draw_text(c, str(value), L.margin_left + 180, y - 4,
                      font_name=FONT_SANS, size=10, color=NAVY)
            y -= 16

        # Motivo de consulta (texto largo, párrafo)
        y -= 4
        draw_text(c, "Motivo de consulta inicial:", L.margin_left, y - 4,
                  font_name=FONT_SANS_BOLD, size=8, color=SLATE)
        y -= 16
        y = draw_paragraph(c, motivo, L.margin_left, y,
                           width=L.page_w - L.margin_left - L.margin_right,
    ...
    def _draw_motivo_cierre(self, c, data, y: float) -> float:
        ...
        y = callout(c, label, y, accent=color)
        y -= 4
        y = draw_paragraph(c, nota, LAYOUT.margin_left, y,
                           width=LAYOUT.page_w - LAYOUT.margin_left - LAYOUT.margin_right,
                           font_name=FONT_SERIF, size=10, color=NAVY)
        return y - 8
    ...
    def _draw_objetivos_evolucion(self, c, data, y: float) -> float:
        ...
        for i, obj in enumerate(objetivos, 1):
            ...
            # Número + descripción
            draw_text(c, f"{i:02d}", L.margin_left, y - 4,
                      font_name=FONT_SERIF_BOLD, size=12, color=PLUM)
            y_text = draw_paragraph(
                c, desc, L.margin_left + 28, y,
                width=L.page_w - L.margin_left - L.margin_right - 28,
                font_name=FONT_SANS, size=10, color=NAVY,
            )
            # Criterios medibles
            if criterios:
                y_text = draw_paragraph(
                    c, f"Criterio: {criterios}", L.margin_left + 28, y_text - 2,
                    width=L.page_w - L.margin_left - L.margin_right - 28,
                    font_name=FONT_SERIF, size=8.5, color=SLATE,
                )
            # Barra de progreso
            y_bar = y_text - 10
            bar_x = L.margin_left + 28
            bar_w = L.page_w - L.margin_left - L.margin_right - 28 - 60
            bar_h = 6
            # Fondo
    ...
    def _draw_sintesis_proceso(self, c, data, y: float) -> float:
        ...
        return draw_paragraph(
            c, sintesis, LAYOUT.margin_left, y,
            width=LAYOUT.page_w - LAYOUT.margin_left - LAYOUT.margin_right,
            font_name=FONT_SERIF, size=10, color=NAVY,
        )
    ...
    def _draw_riesgo_longitudinal(self, c, data, y: float) -> float:
        ...
        for ev in riesgos[:10]:  # máximo 10 entradas
            ...
            c.circle(L.margin_left + 6, y, 4, fill=1, stroke=0)
            draw_text(c, str(fecha)[:10], L.margin_left + 20, y - 3,
                      font_name=FONT_SANS, size=8.5, color=SLATE)
            draw_text(c, nivel.replace("_", " ").title(),
                      L.margin_left + 120, y - 3,
