...
import { HITOS_DESARROLLO } from "../../data/hitosDesarrollo.js";
...
/** Grid de campos de desarrollo — definido a nivel módulo para no perder foco al escribir. */
function DevFieldGrid({ children, title, icon }) {
  return (
    <div className="space-y-4">
      <h4 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2" style={{ color: "var(--ns-muted)" }}>
        {icon && <I name={icon} className="text-base" />}{title}
      </h4>
...
export default function ClinicalHistoryPage({setPage}){
  ...
  const[showConsent,setShowConsent]=useState(false);const[pendCount,setPendCount]=useState(0);const[consentDismissed,setConsentDismissed]=useState(false);const[showHitos,setShowHitos]=useState(false);
  const checkConsents=async(pid,{autoOpen=false}={})=>{if(!pid){setPendCount(0);return}try{const p=await api.get(`/api/v1/consentimientos/pendientes/${pid}`);const pend=Array.isArray(p)?p:(p?.pendientes||[]);setPendCount(pend.length);if(pend.length>0&&autoOpen&&!consentDismissed)setShowConsent(true)}catch{setPendCount(0)}};
  const loadHC=async(pid)=>{if(!pid)return;setLoading(true);try{const d=await api.get(`/api/v1/clinical-history/${pid}`).catch(()=>null);const patData=patients.find(p=>p.id===pid)||null;if(d){const merged={...dflt};Object.keys(d).forEach(k=>{if(k in merged&&d[k]!==null)merged[k]=d[k]});const dec=decodeMC(merged.motivo_consulta||"");merged.mc_remitente=dec.rem;merged.mc_subjetivo=dec.sub;merged.enfermedad_actual=dec.act;if(patData){merged.acompanante_nombre=patData.acompanante||"";merged.acompanante_relacion=patData.acompanante_relacion||"";merged.acompanante_telefono=patData.acompanante_telefono||""}sF(merged)}}catch{}setLoading(false);checkConsents(pid,{autoOpen:true})};
  ...
  const devEsf=[["control_anual","Control de esfínteres (anal)"],["control_vesical","Control de esfínteres (vesical)"]];
  ...
  const esAdulto=patAgeYrs!=null&&patAgeYrs>=18;
  const patPoblacion=patAgeYrs===null?null:patAgeYrs<18?"infantil":patAgeYrs<50?"adulto_joven":"adulto_mayor";
  const viaNav={
    neuropsicologia:{label:"Ir a evaluación",page:"eval_apply"},
    psicoterapia:{label:"Ir a psicoterapia",page:"therapy"},
    rehabilitacion:{label:"Ir a rehabilitación",page:"rehab"},
    mixto:{label:"Ir a screening",page:"screening"},
  };
  const cieSugeridos=patPoblacion?(CIE_POR_POBLACION[patPoblacion]||[]):[];
  return(<><TopBar title="Historia Clínica"><div className="flex items-center gap-3"><PatientSelect bare patients={patients} loading={patientsLoading} value={f.patient_id} onChange={(v)=>{set("patient_id",v);setPatId(v);setConsentDismissed(false);loadHC(v)}} selectClassName="text-xs w-72" placeholder="-- Seleccionar Paciente --" />{loading&&<div className="animate-spin w-5 h-5 border-2 border-teal-200 border-t-teal-600 rounded-full"/>}{f.patient_id&&<button onClick={()=>setShowConsent(true)} className="relative flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-opacity hover:opacity-90" style={{color:pendCount>0?"#b45309":TEAL,background:pendCount>0?"#fef3c7":"var(--ns-subtle)"}} title="Consentimientos informados"><I name={pendCount>0?"error":"verified_user"} className="text-base"/>{pendCount>0?`${pendCount} pendiente${pendCount===1?"":"s"}`:"Consentimientos"}</button>}{/* §QW-4: Imprimir HC PDF */}{f.patient_id&&<button onClick={async()=>{try{const blob=await api.blob(`/api/v1/clinical-history/${f.patient_id}/pdf`,"GET");const u=URL.createObjectURL(blob);window.open(u,"_blank");setTimeout(()=>URL.revokeObjectURL(u),60000)}catch(e){setMsg(_parseError(e))}}} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-teal-50" style={{color:TEAL,background:"var(--ns-subtle)"}} title="Generar PDF de la Historia Clínica completa para imprimir o adjuntar"><I name="print" className="text-base"/>Imprimir HC</button>}</div></TopBar>
  <main className="p-8 max-w-7xl mx-auto space-y-6">
    <MsgBanner msg={msg==="ok"?"ok":msg} onDismiss={msg&&msg!=="ok"?()=>setMsg(""):null}/>
    {f.patient_id&&patSel&&<Card className="p-3 flex flex-wrap items-center gap-4 text-xs" style={{background:"var(--ns-subtle)"}}>
      <span><b>Documento:</b> {patSel.tipo_documento||"CC"} {patSel.numero_documento}</span>
      <span><b>CIE registro:</b> {patSel.codigo_rips||f.codigo_cie10||"—"}</span>
      {patSel.via_atencion&&viaNav[patSel.via_atencion]&&setPage&&<Btn className="ml-auto text-xs" onClick={()=>setPage(viaNav[patSel.via_atencion].page)}>{viaNav[patSel.via_atencion].label}</Btn>}
    </Card>}
    ...
    {tab===3&&<Card className="p-8 space-y-6"><h3 className="text-lg font-bold mb-2 flex items-center gap-2"><I name="assignment" className="text-green-600"/>Plan de Atención</h3>
      <SectionCard title="Screening y tamización" icon="fact_check" eyebrow="Flujo clínico" subtitle="Escalas breves (MMSE, tamización emocional, etc.) se registran en Evaluación → Screening.">
      ...
      <div className="flex flex-wrap gap-2">{patSel?.via_atencion&&viaNav[patSel.via_atencion]&&setPage&&<Btn v="outline" className="text-xs" onClick={()=>setPage(viaNav[patSel.via_atencion].page)}><I name="arrow_forward" className="text-sm"/>{viaNav[patSel.via_atencion].label}</Btn>}{setPage&&<Btn v="outline" className="text-xs" onClick={()=>setPage("eval_apply")}><I name="psychology" className="text-sm"/>Evaluación NPS</Btn>}</div>
  ...
  {showConsent&&f.patient_id&&<ConsentModal patientId={f.patient_id} patientName={patNombre} patientEmail={patSel?.correo||""} onClose={()=>{setShowConsent(false);setConsentDismissed(true);checkConsents(f.patient_id)}} onAllSigned={()=>{setPendCount(0);setConsentDismissed(false)}}/>}
  {showHitos&&<div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:"rgba(0,0,0,0.55)"}} onClick={()=>setShowHitos(false)}><Card className="max-w-lg w-full max-h-[80vh] overflow-auto p-6" onClick={e=>e.stopPropagation()}><h3 className="font-bold mb-3">Hitos de desarrollo (referencia)</h3><table className="w-full text-xs"><thead><tr style={{color:"var(--ns-muted)"}}><th className="text-left py-1">Mes</th><th className="text-left">Motor</th><th className="text-left">Lenguaje</th></tr></thead><tbody>{HITOS_DESARROLLO.map(h=><tr key={h.mes} className="border-t" style={{borderColor:"var(--ns-card-b)"}}><td className="py-2 font-mono">{h.mes}</td><td>{h.motor}</td><td>{h.lenguaje}</td></tr>)}</tbody></table><p className="text-[10px] mt-3" style={{color:"var(--ns-muted)"}}>Referencias: {HITOS_DESARROLLO.map(h=>h.ref).filter((v,i,a)=>a.indexOf(v)===i).join(" · ")}</p><Btn className="mt-4 w-full" onClick={()=>setShowHitos(false)}>Cerrar</Btn></Card></div>}
  </>);
}
