#!/usr/bin/env python3
"""Extract WISC/WAIS protocol text from PDFs for verification."""
import re
import sys
...
def main() -> None:
 ...
 wisc_path = DRIVE / "WISC IV .pdf"
 wais_path = DRIVE / "WAIS III .pdf"
 ...
 samples = {
 "wisc_semejanzas_extract.txt": extract_pdf("WISC IV .pdf", [p - 1 for p in wisc_sem_pages]),
 "wisc_vocabulario_extract.txt": extract_pdf("WISC IV .pdf", [p - 1 for p in wisc_voc_pages]),
 "wisc_digitos_extract.txt": extract_pdf("WISC IV .pdf", [p - 1 for p in wisc_rdd_pages]),
 "wisc_informacion_extract.txt": extract_pdf("WISC IV .pdf", [p - 1 for p in wisc_inf_pages]),
 "wais_semejanzas_extract.txt": extract_pdf("WAIS III .pdf", [p - 1 for p in wais_sem_pages]),
 "wais_vocabulario_extract.txt": extract_pdf("WAIS III .pdf", [p - 1 for p in wais_voc_pages]),
 }
