...
**Verificación PDF IN&S:** `docs/VERIFICACION_INS_PDF_2026-06-05.md` (WISC/WAIS drive-download confirmados ✅)  
...
- [x] Verificación PDF IN&S vs JSON (`VERIFICACION_INS_PDF_2026-06-05.md`)
- [ ] Johan valida que protocolos JSON reflejan su manual físico

### Fase 1 — Verbales con texto completo en JSON (~2-3 h IA)

Generador: `docs/scripts/sync_reactivos_from_protocol.py` (a crear)
...
### Fase 2 — Visuales (Matrices, Conceptos)

- No poner texto verbatim de estímulos gráficos en bundle público.
- UI: componente `StimulusReferencePanel` → "Abrir cuadernillo" + captura 0/1.
- Importar solo `respuestas_correctas` para validación opcional en modo entrenamiento.

### Fase 3 — Tests y regresión

- Test: conteo ítems REACTIVOS == protocolo por `test_id`.
- Test: NiWiscSem par[1] == `Leche — Agua`.
